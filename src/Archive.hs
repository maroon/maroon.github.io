...
yearForIdentifier :: (MonadMetadata m, MonadFail m, Num a)
                  => Identifier
                  -> m a
...
archiveGroup :: (MonadMetadata m, MonadFail m)
             => [Identifier]
             -> m [[Identifier]]
archiveGroup ids = do
  ...
  where
    yearMap :: (MonadMetadata m, MonadFail m)
            => Identifier
            -> m (Integer, Identifier)
...
archivePages :: (MonadMetadata m, MonadFail m)
             => [Identifier]
             -> m PageNumber
archivePages = yearForIdentifier . head

buildArchivePaginateWith :: (MonadMetadata m, MonadFail m)
